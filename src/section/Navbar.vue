<template>
    <div class="flex space-x-96 BSM:space-x-64  meduim:space-x-96 ">
        <svg class=" mt-3 ml-14 absolute top-2 left-4 max-sm:ml-0" width="129" height="29" viewBox="0 0 129 29" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <ul class="flex  mt-3 max-sm:hidden BSM:flex ">
                    <li v-for="N in Nav" class="font-Montserrat text-lg text-Nblack-0 mx-5 BSM:text-sm mt-2 meduim:text-base mx-2  " ><a :href ="N.hre">{{ N.name }}</a></li>
                </ul>
    </div>
...
import { Nav } from '../constant/Data.vue';
</script>