<template >
    <div class="flex flex-row flex-wrap mt-10">
        <div class=" flex flex-col space-y-3">
            <div class=" bg-card bg-cover w-32 h-32 ">
                <img class=" ml-4 mt-1 w-24 h-24 hover:rotate-[17deg]" :src="props.src" alt="">
            </div>
            ...
            <p class="  font-Palanquin font-bold">{{ props.title }}</p>
            <p class=" text-Orange-0" >{{ props.price }}</p>
        </div>
    </div>
</template>
<script setup>
import { defineProps } from 'vue';
const props = defineProps(['src','title','rating','price']);
</script>