<script setup>
import {CustomersInfo} from '../constant/Data.vue'
</script>
<template>
    <div class="flex flex-col lg:mx-10 lg:flex-row md:flex-row">
        <div v-for="items in CustomersInfo" class="flex flex-col space-y-10 mt-10">
            <img :src="items.image" alt="" class=" rounded-full w-20 h-20 flex justify-center items-center mx-auto">
            <p class="  font-Montserrat text-Gray-0 whitespace-pre-wrap text-center">{{ items.comment }}</p>
            <div class="flex flex-row justify-center items-center ">
                ...
                <p class="pl-2">{{ items.rating }}</p>
            </div>
            <p class="text-center font-bold text-xl">{{ items.name }}</p>
        </div>
    </div>
</template>